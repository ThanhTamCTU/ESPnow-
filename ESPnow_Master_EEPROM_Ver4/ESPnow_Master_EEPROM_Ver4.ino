...
//                  } // end if 
//                 else {
//                    Serial.println("Error sending request data SETUP");
//                  }// end if 

                   esp_err_t rs = esp_now_send((uint8_t*)broadcastAddress, (uint8_t *) request_data, sizeof(request_data));
           if( rs== ESP_OK){
                Serial.println(" Đã gửi yêu cầu dữ liệu SETUP");
           }else{
                Serial.println(" chưa gửi yêu cầu dữ liệu SETUP");
            }
...
void OnDataRecv(const uint8_t * mac_addr, const uint8_t *incomingData, int len) {
 ...
 if( switch_request==0){
   // Đoạn code gửi yêu cầu gửi dữ liệu đến slave được chọn 
      esp_err_t result_request_data = esp_now_send((uint8_t*)slave1, (uint8_t *) request_data, sizeof(request_data));
